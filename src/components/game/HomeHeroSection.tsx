...
export function HomeHeroSection({
  ...
  return <div className="relative w-full flex flex-col items-center">
      ...
      <div className="relative w-full max-w-xs h-36 flex items-end justify-center cursor-pointer" onClick={handleTap}>
        {/* Circular glow/spotlight */}
        <div className="absolute bottom-4 w-32 h-32 rounded-full animate-pulse-slow" style={{
        ...
        <div className="relative z-10 mb-2">
          <AnimatedSkinCycler animation={heroAnimation} size={80} interval={3000} showGlow />
        </div>

        {/* Speed lines */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(6)].map((_, i) => <div key={i} className="absolute h-0.5 bg-gradient-to-r from-primary/50 to-transparent animate-speed-line" style={{
          left: '-100%',
          top: `${30 + i * 12}%`,
          width: '60%',
        ...
        {showBoss && <div className="absolute right-0 bottom-4 animate-slide-in-boss" style={{
        opacity: 0.6
      }}>
            <BossSilhouette />
          </div>}
        ...
        {/* Particles */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(8)].map((_, i) => <div key={i} className="absolute w-1.5 h-1.5 rounded-full bg-primary/60 animate-float-particle" style={{
          left: `${10 + i * 12}%`,
          bottom: '20%',
...
function BossSilhouette() {
  return <svg width="48" height="48" viewBox="0 0 48 48" className="text-red-500">
      {/* Boss body - spiky demon shape */}
      <path d="M24 4 L28 12 L36 8 L32 16 L44 20 L32 24 L36 32 L28 28 L24 44 L20 28 L12 32 L16 24 L4 20 L16 16 L12 8 L20 12 Z" fill="currentColor" opacity="0.8" />
      {/* Glowing eyes */}
